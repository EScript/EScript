...
// ---
{
    ...
    if(n==1 && i==3 && (0x01+255).toHex()=="0x100" && "-1.7".toNumber()==-1.7 && 1.getType()==Number
			...
			&& 1.clamp(5,10)==5 && 20.clamp(1,2)==2 && 1.893.clamp(1,2)==1.893
		)
...
//---
{
	...
	// out(a);
	if(a == 53){out (OK);}else { errors+=1; out(FAILED); }
}
//---
{
	out("Blocks:\t\t");
...
//	print_r(a);
	...
	if(plusRec(a,7)==17 && plusRec2(a,7)==17 && minusOne(a)==9 && (fn(a){return a*a;})(2)==4 &&
...
//	out(ar.max());
	...
	if( accum=="1827Hoobelbarding18bardidu" && a ---|> Array && ! (1 ---|> Array)
			&& (new Array(1,'a','b')).implode(',')=='1,a,b' && c.implode()=='1234'
...
//---
{
	...
	var m2={1:2,3:4};
	out(m2[1]);
	m1!={"foo":"bar","bla":[1,2,3,4]};


	if( accum=="|bla:dada|dum:dada|foo:barbar3"
			&& m1=={"foo":"bar","bla":[1,2,3]}	&& m1!={"foo":"bar","bla":[1,2,3,4]}
			&& ({:} ---|> Map) && ! (({})---|> Map )
...
{
    out("Parser:\t\t");
    var s=Parser.parse("while(a<10)a++;");
    var a=0;
    s.execute();
    //out(a);
    if( a==10)
...
//---
{
    ...
    var d2=a -> fn(){return this.m1;}; // EXPERIMENTAL!
...
//---
{
    ...
    var jsonList=_toJSON(list,false);
//    out("\n",jsonList);
//    print_r(list);
//    print_r(_parseJSON(jsonList));

    if(list==_parseJSON(jsonList))
        {out (OK);}else { errors+=1; out(FAILED); }
}
// ---
{
