...
//
	...
	//! [ESMF] self Array.filter(function)
	ESMF_DECLARE(typeObject,Array,"filter",1,1,(self->rt_filter(runtime,parameter[0]),self))
...
void Array::rt_filter(Runtime & runtime,ObjPtr function) {
    ...
    for (vector<ObjRef>::const_iterator it=vec.begin();it!=vec.end();++it) {
        ObjRef resultRef=callFunction(runtime,function.get(),ParameterValues(*it));
        if( resultRef.toBool() ){
