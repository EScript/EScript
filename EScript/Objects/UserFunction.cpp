...
/*!	(ctor)	*/
UserFunction::UserFunction(parameterList_t * _params,Block * block,bool _lambda):
		params(_params),lambda(_lambda) {

	setBlock(block);
    //ctor
}
/*!	(ctor)	*/
UserFunction::UserFunction(parameterList_t * _params,Block * block,bool _lambda,const std::vector<ObjRef> & _sConstrExpressions):
		params(_params),sConstrExpressions(_sConstrExpressions.begin(),_sConstrExpressions.end()),lambda(_lambda) {
...
void UserFunction::setBlock(Block * _block){
	blockRef=_block;
//	if(!blockRef.isNull()){ // \todo is this necessary? if not, remove it
//	    for (parameterList_t::iterator it=params->begin();it!=params->end();++it) {
//			blockRef->declareVar( (*it)->getName());
//		}
//	}
}
