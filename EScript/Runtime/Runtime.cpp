 ...
 */
ESF(esf_Runtime_setErrorConfig,1,1, ( runtime.setErrorConfig(static_cast<unsigned int>(parameter[0].toInt())),Void::get()) )

/**
 * [ESMF] number Runtime.esf_Runtime_getErrorConfig();
 */
ESF(esf_Runtime_getErrorConfig,0,0,Number::create(runtime.getErrorConfig()))

/**
 * [ESMF] String Runtime.getStackInfo();
 */
ESF(esf_Runtime_getStackInfo,0,0,String::create(runtime.getStackInfo()))
...
//
    ...
    declareFunction(typeObject,"_setErrorConfig",esf_Runtime_setErrorConfig); // working title
    declareFunction(typeObject,"_getErrorConfig",esf_Runtime_getErrorConfig); // working title
    declareFunction(typeObject,"getStackInfo",esf_Runtime_getStackInfo); // working title
...
Runtime::Runtime():ExtObject(Runtime::typeObject), state(STATE_NORMAL),errorConfig(0),currentLine(0) {
    ...
    globals=Script::SGLOBALS.get()->clone();
    globals->setObjAttribute(stringToIdentifierId( "GLOBALS"),globals.get());
    globals->setObjAttribute(stringToIdentifierId( "SGLOBALS"),Script::SGLOBALS.get());
    //ctor
}
...
////////	    // \note getInheritedAttribute is used to skip the members of Type
 ...
 *
 */
void Runtime::initVariable(const identifierId id,Object * value) {
    // search for local variable (bla)
    RuntimeBlock * rtb = getCurrentRTB();
    if (rtb) {
        rtb->initLocalVariable(id,value);
    } else {
        globals->setObjAttribute(id,value);
    }
}
...
Object * Runtime::executeUserFunction(RuntimeBlock * rtb,const ObjPtr & _callingObject,bool lambda) {
    ...
    bool noReturn=true;
    if(!checkNormalState()){
        if(getState()==Runtime::STATE_RETURN){
            resultRef=returnRef;
            resetState();
            noReturn=false;
...
Object * Runtime::executeUserConstructor(const ObjPtr & _callingObject,const ParameterValues & params){
	...
	while(!consCallStack.empty()){
		...
		ObjRef tmp = executeUserFunction(rtb,baseObj,false); // here the RTB is poped and destroyed...
